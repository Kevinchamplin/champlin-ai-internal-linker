...
### Changed (2026-07-17, wporg-cil-to-chail-prefix-and-enqueue) [1.5h]
- **v1.3.4 — cleared the second WordPress.org review round (short prefix + inline `<style>`).** The 2026-07-17 reviewer flagged (a) the 3-char `cil`/`CIL_` prefix as too short (>4 chars required) and (b) an inline `<style>` block in `src/Admin/UpgradeToProPanel.php`. The review also re-listed the Pro-installer + `linkweaver.app/legal` 404, but those were already resolved in v1.3.3 — the reviewer had scanned the older v1.3.2 upload. Both live issues are fixed for real (not suppressed):
  - **Prefix rename `cil`/`CIL_` → `chail`/`CHAIL_`** across every identifier: constants, action/filter hooks, options, transients, cron hooks, the REST route namespace (`chail/v1`), script + style handles, JS localize objects, CSS classes, and the two custom tables (`wp_chail_embeddings`, `wp_chail_suggestion_log`). Public extension filters are now `chail_provider`, `chail_provider_summary`, `chail_settings_render_extra`, `chail_extra_excluded_ids`, `chail_rank_results`, `chail_suggestion_row`. Configured `chail` as the allowed prefix in `phpcs.xml` and removed the now-needless inline `phpcs:disable/ignore …PrefixAllGlobals` suppressions — verified `PrefixAllGlobals` reports **0** violations.
  - **Inline `<style>` → enqueued CSS.** Moved the "Upgrade to Pro" panel rules into `assets/admin/admin.css` (shipped via the existing `wp_enqueue_style`), deleted the `<style>` block.
  - Repointed the panel's dead `linkweaver.app/account` link (404) to `linkweaver.app/` (200); fixed a stale `text_domain` typo in `phpcs.xml`.
  - No behavior change. Plugin was never released, so the table/option rename needs no migration. Built + audited `dist/champlin-ai-internal-linker.zip` (v1.3.4, 73 files): 0 `cil` tokens, 0 inline `<style>`, PUC stripped. **Kevin must upload this zip to the existing WP.org review ticket + reply to the review email.**
