...
Inside your WordPress database, in two custom tables (`wp_chail_embeddings` and `wp_chail_suggestion_log`). Both tables are dropped automatically when the plugin is deleted.
...
= 1.3.4 =
* Change: Renamed every internal identifier from the short `cil`/`CIL_` prefix to `chail`/`CHAIL_` — constants, action/filter hooks, options, transients, script and style handles, the REST route namespace, and the two custom database tables — so nothing can collide with other plugins. The public extension filters are now `chail_provider`, `chail_provider_summary`, `chail_settings_render_extra`, `chail_extra_excluded_ids`, `chail_rank_results`, and `chail_suggestion_row`.
* Change: Moved the "Upgrade to Pro" panel styles out of an inline `<style>` tag into the enqueued admin stylesheet.
...
= 1.3.1 =
* Fix: Hosted-AI installs (Premium with no site-level OpenAI key) could not index — `is_configured()` only checked for a site API key and bailed before the `chail_provider` filter (which supplies the hosted provider) was considered. Indexing + suggestions now work when the embedding provider is injected via the filter.
...
* Add: `chail_settings_render_extra` action — Pro and add-ons can inject custom settings panels.
* Add: `chail_provider` filter — Pro's HostedProvider can drop in as the embedding source when a license is active.
* Add: `chail_provider_summary` filter — Pro can tell Free's Settings page that hosted AI is active.
* Add: `chail_extra_excluded_ids`, `chail_rank_results`, `chail_suggestion_row` filters for Pro to extend ranking + per-row display data.
...
* Fix: Reports → Orphans now ships an inline "How to fix" workflow with top-3 inbound-candidate posts per orphan, computed from existing embeddings. Each candidate has a one-click "Open in editor" link with `?chail_open=1` that auto-opens the AI Linker sidebar.
...
* Add: REST routes `GET /chail/v1/reports/orphans` and `POST /chail/v1/reports/rescan` (admin capability).
